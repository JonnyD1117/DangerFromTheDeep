...
{
    unsigned w, h, n;
    sorty(unsigned w_, unsigned h_, unsigned n_)
        : w(w_)
        , h(h_)
        , n(n_)
    {
    }
    bool operator<(const sorty& s) const { return (h > s.h) ? true : (h == s.h ? w > s.w : false); }
};

struct position
{
    unsigned x, y;
    position(unsigned x_ = 0, unsigned y_ = 0)
        : x(x_)
        , y(y_)
    {
    }
...
void init(const char* fname, unsigned int h, string outputfilename)
{
    ...
    if (FT_New_Face(library, fname, 0, &face))
        throw std::runtime_error("FT_New_Face failed (there is probably a "
                                 "problem with your font file)");
    ...
    for (unsigned texw = 64; texw <= 2048; texw <<= 1) {
        ...
        if (sumh > 0) {
            ...
            std::cout << "texture " << texw << "x" << texh << " (h=" << sumh << ") with wasted pixels: " << xwaste
                      << " + " << ywaste << " = " << xwaste + ywaste << "\n";
        }
    }

    // now allocate texture and fill in characters, using min_waste_positions
    // data
        ...
        if (sorties[i].w > 0) {
            ...
            for (unsigned y = 0; y < bh[j]; ++y) {
                for (unsigned x = 0; x < bw[j]; ++x) {
                    all[min_waste_positions[i].x + x + (min_waste_positions[i].y + y) * min_waste_texw] =
                        bd[j][y * bw[j] + x];
                }
            }
        } else {
