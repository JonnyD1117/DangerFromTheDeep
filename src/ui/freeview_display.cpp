...
auto freeview_display::get_projection_data(game& gm) const -> freeview_display::projection_data
{
    projection_data pd;
    pd.x          = 0;
    pd.y          = 0;
    ...
    , conning_tower(nullptr)
{
    ...
    conning_tower = ui_.get_game().get_model_store().ref(sub->get_bridge_filename());

    texture_store().ref("splashring.png");
    ...
    underwater_background = texture_store().create(
        "underwater_background.png", get_texture_dir() + "underwater_background.png", texture::LINEAR, texture::CLAMP);
}

freeview_display::~freeview_display() = default;
...
auto freeview_display::handle_key_event(const key_data& k) -> bool
{
    if (k.down()) {
        glPushMatrix();
        glLoadIdentity();
        set_modelview_matrix(ui.get_game(),
...
auto freeview_display::handle_mouse_motion_event(const mouse_motion_data& m) -> bool
{
    if (m.left()) {
...
auto freeview_display::handle_mouse_wheel_event(const mouse_wheel_data& m) -> bool
{
    glPushMatrix();
    glLoadIdentity();
    ...
    bool mirrorclip) const
{
    ...
    if (withunderwaterweapons) {
        ...
        for (const auto* it : depth_charges) {
            glPushMatrix();
    ...
    // compare function
    for (const auto* water_splashe : water_splashes) {
        glPushMatrix();
...
void freeview_display::draw_view(game& gm, const vector3& viewpos) const
{
    ...
    GLfloat horizon_color[4] = {
        0.050980392156862744F, 0.054901960784313725F, 0.27450980392156865F, 0.0F /*this is bad*/};
    ...
    GLfloat lposition[4] = {
        static_cast<GLfloat>(sundir.x), static_cast<GLfloat>(sundir.y), static_cast<GLfloat>(sundir.z), 0.0F};
    ...
    GLfloat lambient[4] = {
        ambient_intensity * lightcol.r, ambient_intensity * lightcol.g, ambient_intensity * lightcol.b, 1.0F};
    ...
    for (const auto* object : objects) {
        const auto* s = dynamic_cast<const ship*>(object);
        if (s != nullptr) {
            const auto* t = dynamic_cast<const torpedo*>(s);
            if (t == nullptr) {
    ...
    draw_objects(gm, viewpos, objects, lightcol, above_water < 0 /* under water */, false /* mirrorclip */);
