...
void coastsegment::cacheentry::push_back_point(const vector2& p)
{
    if (!points.empty()) {
        ...
        ASSERT(
            d >= 1.0f, "error: points are too close %f    %f %f  %f %f", d, points.back().x, points.back().y, p.x, p.y);
    }
    points.push_back(p);
}
...
void coastsegment::generate_point_cache(const class coastmap& cm, int x, int y, int detail) const
{
    if (type > 1) {
        ...
        if (!pointcache.empty() && pointcachedetail == detail) {
            return;
        }
        ...
        for (unsigned i = 0; i < nrcl; ++i) {
            ...
            // if (dbl > 0) cout << "erased " << dbl << " double points!, seg "
            // << x << "," << y << "\n";
...
void coastsegment::draw_as_map(const class coastmap& cm, int x, int y, int detail) const
{
    ...
    if (type == 1) {
        ...
        primitives::textured_quad(vector2f(0, 0), vector2f(cm.segw_real, cm.segw_real), *atlanticmap, tc0, tc1)
            .render();
    } else if (type > 1) {
        vector2f tc0 = cm.segcoord_to_texc(x, y);
        vector2f tc1 = cm.segcoord_to_texc(x + 1, y + 1);
        ...
        unsigned nrv = 0;
        for (const auto& p : pointcache) {
            nrv += p.indices.size();
        }
        primitives tris(GL_TRIANGLES, nrv, color::white(), *atlanticmap);
        nrv = 0;
        for (const auto& p : pointcache) {
            for (auto& t : p.indices) {
                const vector2& v = p.points[t];
...
auto coastmap::borderpos(const coastsegment::segpos& p) -> int
{
    if (p.y == 0) {
        return p.x;
    }
...
void coastsegment::segcl::print() const
{
    ...
    */
    std::cout << "segcl:\n"
              ...
              << " beginb " << ((beginpos == -1) ? -1 : beginpos / int(SEGSCALE)) << " endpos " << endpos << " endb "
              << ((endpos == -1) ? -1 : endpos / int(SEGSCALE)) << " next " << next << " cyclic? " << cyclic
              << " nrpts " << points.size() << " 1st pt " << points.front() << ",back " << points.back() << "\n";
}
...
auto coastmap::find_coastline(int x, int y, std::vector<vector2i>& points, bool& cyclic) -> bool
{
...
auto coastmap::compute_segment(const vector2i& p0, const vector2i& p1) -> vector2i
{
...
void coastmap::divide_and_distribute_cl(const std::vector<vector2i>& cl, bool clcyclic)
{
    ...
    for (unsigned i = 1; i < cl.size();) {
        ...
        if (rel.x >= 0 && rel.y >= 0 && rel.x <= int(SEGSCALE) && rel.y <= int(SEGSCALE)) {
            ...
            coastsegment::segpos ps2(static_cast<unsigned short>(rel.x), static_cast<unsigned short>(rel.y));
...
auto coastmap::segcoord_to_real(int segx, int segy, const coastsegment::segpos& sp) const -> vector2
{
...
coastmap::coastmap(const std::string& filename, object_store<model>& model_store_)
    : model_store(model_store_)
{
    ...
    if (er.has_child("cities")) {
        for (auto elem : er.child("cities").iterate("city")) {
            cities.emplace_back(
                vector2(
                    transform_nautic_posx_to_real(elem.attr("posx")), transform_nautic_posy_to_real(elem.attr("posy"))),
                elem.attr("name"));
        }
    }
    ...
    if (er.has_child("props")) {
        for (auto elem : er.child("props").iterate("prop")) {
            ...
            auto m              = model_store.ref(path);
            m->register_layout();
            m->set_layout();
            props.emplace_back(path, vector2(elem.attrf("posx"), elem.attrf("posy")), elem.attrf("angle"));
        }
    }

    {
        ...
        if (segsx * pixels_per_seg != mapw || segsy * pixels_per_seg != maph) {
            THROW(
                error,
                std::string("coastmap: map size must be integer multiple of segment "
                            "size, in")
                    + filename);
        }
    ...
    myworker = std::make_unique<::thread>("coastmap-worker", [this]() { construction_threaded(); });
}

coastmap::~coastmap() = default;
...
void coastmap::render(const vector2& p, double vr, bool mirrored, int /*detail*/, bool /*withterraintop*/) const
{
    ...
    for (const auto& prp : props) {
        if (prp.pos.square_distance(p) < vr * vr) {
            // potentially visible
            glPushMatrix();
            glTranslatef(prp.pos.x - p.x, prp.pos.y - p.y, 0); //- p.z
            glRotatef(-prp.dir, 0, 0, 1);
            if (mirrored) {
                // fixme: display_mirror_clip must be called with
                // certain conditions, that are not used here yet...
                model_store.ref(prp.modelname)->display_mirror_clip();
            } else {
                model_store.ref(prp.modelname)->display();
            }
            glPopMatrix();
        }
    }
