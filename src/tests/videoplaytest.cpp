...
videoplay::videoplay(const std::string& filename, unsigned queue_len)
    ...
    , eof(false)
    , next_frame_pts(-1.0)
    , myshader(get_shader_dir() + "videoplay.vshader", get_shader_dir() + "videoplay.fshader")
{
...
void videoplay::loop()
{
    ...
    double stamp = double((ipkt.pts == AV_NOPTS_VALUE) ? ipkt.dts : ipkt.pts) * ictx->streams[vstr_idx]->time_base.num
...
void videoplay::display_loop(double delta_time)
{
    ...
    unsigned nr_full_buffers = 0;
    {
        ...
        if (!decoded_pictures.empty()) {
            ...
            if (playback_started && fb_queue[bufnr].time_stamp <= (real_time + playback_starttime))
...
int mymain(std::vector<string>& args)
{
    ...
    bool paused = false;
    bool quit   = false;
    auto vpl    = std::make_unique<videoplay>(filename);
