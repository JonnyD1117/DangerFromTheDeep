...
class tree_generator
{
  ...
  protected:
    ...
    void
    generate_tree(model::mesh& msh, model::mesh& mshleaves, unsigned lvl, const vector3f& root, const vector3f& axis)
        const;
};
...
std::unique_ptr<model> tree_generator::generate() const
{
    ...
    bmap->set_texture(
        new texture(get_texture_dir() + "treebarktest_normal.png", texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    ...
    bool generate_leaves) const
{
    ...
    for (unsigned i = 1; i < 3; ++i) {
        vector3f a = root + axis * (float(i) / 3 * length) + xaxis * ((rnd() - 0.5) * bend_factor * length)
    ...
    const vector3f& axis) const
{
    ...
    vector3f rootc = generate_log(
        msh, mshleaves, lvl, root, axis, segs[lvl], len, radius0[lvl], radius1[lvl], bendfac[lvl], 4, lvl == 4);
