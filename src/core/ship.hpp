...
class ship : public sea_object
{
  ...
  protected:
    ...
    struct generic_rudder
    {
        ...
        double angle;    ///< current rudder angle in degrees, do not use class angle
    ...
    /// area etc.)
    [[nodiscard]] virtual double get_turn_accel_factor() const { return 20000.0; }
    ...
    */
    ...
    static bool is_target_in_blindspot(const struct gun_turret* gun, angle bearingToTarget);
    static bool calculate_gun_angle(const double distance, angle& elevation, const double initial_velocity);
  ...
  public:
    ...
    ship(date game_date, object_store<model>& model_store, const xml_elem& parent);
    ...
    */
    ...
    [[nodiscard]] virtual const std::list<prev_pos>& get_previous_positions() const { return previous_positions; }
    ...
    [[nodiscard]] unsigned calc_damage() const override; // returns damage in percent (100 means dead)
    ...
    [[nodiscard]] virtual double get_max_speed() const { return max_speed_forward; };
    virtual throttle_status get_throttle() { return (throttle_status) throttle; }
    [[nodiscard]] virtual double get_throttle_speed() const;
    [[nodiscard]] virtual double get_throttle_accel() const; // returns acceleration caused by current throttle
    [[nodiscard]] virtual bool screw_cavitation() const;     // returns true if screw causes cavitation
    ...
    [[nodiscard]] static angle estimate_angle_on_the_bow(angle target_bearing, angle target_heading);
    ...
    // sonar
    [[nodiscard]] virtual const noise_signature& get_noise_signature() const { return noise_sign; }
