...
depth_charge::depth_charge(object_store<model>& model_store)
    : sea_object("depth_charge.ddxml", model_store)
    , explosion_depth(0)
{
    // for loading
}

depth_charge::depth_charge(double expl_depth, const vector3& pos, object_store<model>& model_store)
    : sea_object("depth_charge.ddxml", model_store)
