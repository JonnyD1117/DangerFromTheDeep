...
auto sensor::is_within_detection_cone(const vector2& r, const angle& h) const -> bool
{
...
lookout_sensor::lookout_sensor(lookout_type /*type*/)
{
}

auto lookout_sensor::is_detected(const game* gm, const sea_object* d, const sea_object* t) const -> bool
{
    bool detected        = false;
...
auto lookout_sensor::is_detected(const game* gm, const sea_object* d, const particle* p) const -> bool
{
    bool detected        = false;
    double max_view_dist = gm->get_max_view_distance();
...
passive_sonar_sensor::passive_sonar_sensor(passive_sonar_type type)
{
    init(type);
}
    ...
    const sea_object* t) const -> bool
{
    ...
    if ((sub != nullptr) && !sub->is_submerged()) {
        detected = false;
    } else {
...
auto passive_sonar_sensor::is_detected(const game* gm, const sea_object* d, const sea_object* t) const -> bool
{
    double sound_level;
...
// Class active_sensor
active_sensor::active_sensor(double range)
    : sensor(range)
{
}
...
// Class radar_rensor
radar_sensor::radar_sensor(radar_type type)
{
    init(type);
}
...
auto radar_sensor::is_detected(const game* /*gm*/, const sea_object* d, const sea_object* t) const -> bool
{
    if (t == nullptr) {
    ...
    if (((dsub != nullptr) && !dsub->is_submerged()) || (dsub == nullptr)) {
        vector2 r = t->get_pos().xy() - d->get_pos().xy();
        if (is_within_detection_cone(r, d->get_heading())) {
            double df  = get_distance_factor(r.length());
            double vis = 1.0F;
...
auto active_sonar_sensor::is_detected(const game* gm, const sea_object* d, const sea_object* t) const -> bool
{
    bool detected = false;

    // Surfaced submarines cannot use ASDIC.
    const auto* dsub = dynamic_cast<const submarine*>(d);
    if ((dsub != nullptr) && !dsub->is_submerged()) {
        detected = false;
    } else {
        // Only submerged submarines can be detected with ASDIC.
        const auto* tsub = dynamic_cast<const submarine*>(t);
        if ((tsub != nullptr) && tsub->is_submerged()) {
            vector2 r = t->get_pos().xy() - d->get_pos().xy();
