...
#include "game.hpp"
#include "global_data.hpp" // for myfrac etc.
...
auto particle::make_2d_smoothed_noise_map(unsigned wh) -> vector<uint8_t>
{
    ...
    for (unsigned y = 0; y < wh; ++y) {
        ...
        for (unsigned x = 0; x < wh; ++x) {
            ...
            unsigned r = (unsigned(tmp[y1 * wh + x1]) + unsigned(tmp[y1 * wh + x2]) + unsigned(tmp[y2 * wh + x1])
                          + unsigned(tmp[y2 * wh + x2]))
                             / 16
                         + (unsigned(tmp[y * wh + x1]) + unsigned(tmp[y * wh + x2]) + unsigned(tmp[y1 * wh + x])
                            + unsigned(tmp[y2 * wh + x]))
                               / 8
...
auto particle::interpolate_2d_map(const vector<uint8_t>& mp, unsigned res, unsigned x, unsigned y, unsigned res2)
    -> unsigned
{
...
auto particle::make_2d_perlin_noise(unsigned wh, unsigned highestlevel) -> vector<uint8_t>
{
    unsigned whlevel = 0;
...
auto particle::compute_fire_frame(unsigned wh, const vector<uint8_t>& oldframe) -> vector<uint8_t>
{
...
void particle::init()
{
    ...
    for (unsigned i = 0; i < NR_OF_SMOKE_TEXTURES; ++i) {
        ...
        */
        ...
        tex_smoke[i] = new texture(smoketmp, 64, 64, GL_LUMINANCE_ALPHA, texture::LINEAR_MIPMAP_LINEAR, texture::CLAMP);
    }
    ...
    tex_spray = new texture(smoketmp, 64, 64, GL_LUMINANCE_ALPHA, texture::LINEAR_MIPMAP_LINEAR, texture::CLAMP);
...
#define FIRE_RES 64
    ...
    for (unsigned i = 0; i < NR_OF_FIRE_TEXTURES; ++i) {
        ...
        tex_fire[i] = new texture(tmp, FIRE_RES, FIRE_RES, GL_RGBA, texture::LINEAR, texture::CLAMP);
...
smoke_particle::smoke_particle(const vector3& pos)
    : particle(pos)
    , texnr(rand() % NR_OF_SMOKE_TEXTURES)
{
...
auto smoke_particle::get_tex_and_col(game& /*gm*/, const colorf& light_color, colorf& col) const -> const texture&
{
...
explosion_particle::explosion_particle(const vector3& pos)
    : particle(pos)
{
...
auto explosion_particle::get_tex_and_col(game& /*gm*/, const colorf& /*light_color*/, colorf& col) const
    -> const texture&
{
    col    = colorf(1, 1, 1, 1);
...
fire_particle::fire_particle(const vector3& pos)
    : particle(pos)
{
}
...
auto fire_particle::get_tex_and_col(game& /*gm*/, const colorf& /*light_color*/, colorf& col) const -> const texture&
{
    col    = colorf(1, 1, 1, 1);
...
auto spray_particle::get_tex_and_col(game& /*gm*/, const colorf& light_color, colorf& col) const -> const texture&
{
    ...
    if (life > 0.666) {
        ...
        for (unsigned i = 0; i < flares.size(); ++i) {
            ...
            for (unsigned k = 0; k < fls; ++k) {
                ...
                vector3 p =
                    position - viewpos + dx * (f.velocity.x * lifefac3 * 2.0) + dy * (f.velocity.y * lifefac3 * 2.0);
        ...
        for (unsigned i = 0; i < flares.size(); ++i) {
            const flare& f = flares[i];
            vector3 p =
                position - viewpos + dx * (f.velocity.x * lifefac2 * 2.0) + dy * (f.velocity.y * lifefac2 * 2.0);
...
auto fireworks_particle::get_tex_and_col(game& /*gm*/, const colorf& /*light_color*/, colorf& /*col*/) const
    -> const texture&
{
...
marker_particle::marker_particle(const vector3& pos)
    : particle(pos)
{
}
...
auto marker_particle::get_tex_and_col(game& /*gm*/, const colorf& /*light_color*/, colorf& col) const -> const texture&
{
    col = colorf(1, 1, 1, 1);
    return *tex_marker;
}
