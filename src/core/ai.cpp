...
ai::ai(types type_)
    : type(type_)
    , state(followpath)
    , zigzagstate(1 /*0 fixme*/)
    , attackrun(false)
    , evasive_manouver(false)
    , rem_manouver_time(0)
    , has_contact(false)
...
void ai::act(ship& parent, class game& gm, double delta_time)
{
    if (remaining_time <= -1000.0) {
        // set first value, can't be done in c'tor because c'tor has no game
        // object
...
void ai::act_escort(ship& parent, game& gm, double delta_time)
{
    ...
    if (nearest_contact != nullptr) { // is there a contact?
        ...
        if (gm.is_valid(myconvoy)) {
            gm.get_convoy(myconvoy).add_contact(gm, nearest_contact->get_pos());
        }
    ...
    if (!attackrun) { // nothing found? try a ping or listen
        ...
        if (!hearable_subs.empty()) {
            ...
            if (!contacts.empty()) {
                ...
                if (gm.is_valid(myconvoy)) {
                    gm.get_convoy(myconvoy).add_contact(gm, contacts.front());
                }
    ...
    if (state == followpath || state == followobject) {
        ...
        if (cd > DC_ATTACK_RUN_RADIUS && !attackrun) {
            ...
            if (!contacts.empty()) { // update contact
                // fixme: choose best contact!
                if (gm.is_valid(myconvoy)) {
                    gm.get_convoy(myconvoy).add_contact(gm, contacts.front());
                }
            ...
            set_zigzag(cd > 500 && cd < 2500); // zig-zag near the sub - fixme test hack, doesn't
                                               // work
        } else {
            parent.set_throttle(ship::aheadflank);
            attackrun = true;
        ...
        if (cd < DC_ATTACK_RADIUS) {
            ...
            gm.spawn(depth_charge(-contact.z, parent.get_pos(), gm.get_model_store()));
...
auto ai::set_course_to_pos(ship& parent, game& /*gm*/, const vector2& pos) -> bool
{
    ...
    if (r2 > r1) {   // target can not be reached with smallest curve
                     // possible
        if (b < 0) { // target is left
            main_course = parent.get_heading() + angle(180);
            parent.head_to_course(main_course, 1);
