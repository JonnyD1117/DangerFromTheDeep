    ...
    switch (state) {
        ...
        case find_growing_signal:
            if (signal.first > current_signal_strength) {
                ...
                advance_angle_and_erase_old_contacts(
                    (turn_speed_fast - sub_turn_velocity) * simulation_step, sub_heading);
            }
            current_signal_strength = signal.first;
            break;
        ...
        case find_max_peak_fine:
            if (signal.first > current_signal_strength) {
                ...
                auto add = angle(
                    (((find_peak_try & 1) != 0U ? 1 : -1) * turn_speed_slow - sub_turn_velocity) * simulation_step);
...
auto sonar_operator::keeps_in_find_peak_limit(angle add) -> bool
{
    ...
    if (find_peak_lower_limit.is_clockwise_nearer(new_ang) && new_ang.is_clockwise_nearer(find_peak_upper_limit)) {
...
void sonar_operator::save(xml_elem& parent)
{
