...
/* how its done:
   ...
   * delta_t delta_t can be computed, it is the the time a signal needs to
   ...
   has 100 strips with 17µs delay between each strip given a total maximum delay
...
auto noise::compute_ambient_noise_strength(double seastate) -> noise
{
    ...
    for (unsigned band = 0; band < NR_OF_FREQUENCY_BANDS; ++band) {
        result.frequencies[band] = pow(dB_base, background_noise[band] + seastate_factor[band] * seastate);
    }
    return result;
}

auto noise_signature::compute_signal_strength(double distance, double speed, bool caviation) const -> noise
{
    noise result;
...
auto compute_signal_strength_GHG(angle signal_angle, double frequency, angle apparatus_angle) -> double
{
#if 1
