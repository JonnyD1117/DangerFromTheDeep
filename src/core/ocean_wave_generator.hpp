...
T ocean_wave_generator<T>::phillips(const vector2t<T>& K) const
{
    ...
    T result =
        wave_height_scale * eterm * KdotWhat * exp(-k2 * l2); // only here the term "a" (wave height scalar) is used -
...
{
    const T pi2 = T(2.0 * constant::PI);
    for (int y = 0; y <= N / 2; ++y) { // this means one sin/cos per value for every fft, so maybe this
                                       // should go to GPU as well!
        for (int x = 0; x < N; ++x) {
...
ocean_wave_generator<T>::ocean_wave_generator(const ocean_wave_generator<T>& owg)
    ...
    , W(owg.W)
    , v(owg.v)
    , wave_height_scale(owg.wave_height_scale)
    , Lm(owg.Lm)
    , w0(owg.w0)
    , h0tilde(owg.h0tilde)
    , rndgen(12345)
...
template<class T>
ocean_wave_generator<T>::ocean_wave_generator(const ocean_wave_generator<T>& owg, int gridsize, int clearlowfreq)
...
template<class T>
void ocean_wave_generator<T>::compute_finite_normals(const std::vector<T>& heights, std::vector<vector3t<T>>& normals)
    const
{
    if (normals.size() != N * N)
        normals.resize(N * N);
    ...
    for (int y = 0; y < N; ++y) {
        ...
        for (int x = 0; x < N; ++x) {
            ...
            vector3t<T> ortho =
                (a.cross(b) + b.cross(c) + c.cross(d) + d.cross(e) + e.cross(f) + f.cross(g) + g.cross(h) + h.cross(a));
            *it++ = ortho.normal();
        }
    }
}
...
template<class T>
void ocean_wave_generator<T>::compute_displacements(const T& scalefac, std::vector<vector2t<T>>& wavedisplacements)
    const
{
    for (int y = 0; y <= N / 2; ++y) {
        for (int x = 0; x < N; ++x) {
