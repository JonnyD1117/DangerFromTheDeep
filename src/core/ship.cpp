...
ship::ship(date game_date, object_store<model>& model_store, const xml_elem& parent)
    : sea_object(game_date, model_store, parent)
    , tonnage(0)
    , throttle(0)
    , rudder(
    ...
    , maximum_gun_range(0.0)
{
    ...
    if (parent.has_child("ai")) {
        ...
        if (aitype == "dumb") {
            myai = std::make_unique<ai>(ai::dumb);
        } else if (aitype == "escort") {
            myai = std::make_unique<ai>(ai::escort);
        } else if (aitype == "none") {
            myai.reset();
        } else {
    ...
    if (mymodel != nullptr) {
        max_flooded_mass = mymodel->get_base_mesh().volume * constant::SEA_WATER_DENSITY;
...
auto ship::bearing_and_range_to(const sea_object* other) const -> pair<angle, double>
{
    vector2 diff = other->get_pos().xy() - position.xy();
    return make_pair(angle(diff), diff.length());
}

auto ship::estimate_angle_on_the_bow(angle target_bearing, angle target_heading) -> angle
{
...
void ship::simulate(double delta_time, game& gm)
{
    ...
    if (is_inactive()) {
        ...
        const auto& voxdat = mymodel->get_voxel_data();
...
void ship::steering_logic()
{
    ...
    // head_to_fixed value.
    if ((head_to_fixed & HEAD_TO_FORCE_DIRECTION) != 0) {
        if (heading.diff_in_direction((head_to_fixed & HEAD_TO_LEFT) != 0, head_to) >= 180.0) {
            double rudderval = (head_to_fixed & HEAD_TO_ALLOW_HARD_RUDDER) != 0 ? 1.0 : 0.5;
            rudder.set_to((head_to_fixed & HEAD_TO_LEFT) != 0 ? -rudderval : rudderval);
            return;
        }
    ...
    */
    ...
    // error);
    double clamp = (head_to_fixed & HEAD_TO_ALLOW_HARD_RUDDER) != 0 ? 1.0 : 0.5;
    double rd    = myclamp(error / 5.0, -clamp, clamp);
    rudder.set_to(rd);
    // set desired direction, so the 180 degree check code above doesn't abort
    head_to_fixed =
        head_to_param((head_to_fixed & HEAD_TO_ALLOW_HARD_RUDDER) | (rd < 0 ? HEAD_TO_LEFT : HEAD_TO_RIGHT));
...
void ship::compute_force_and_torque(vector3& F, vector3& T, game& gm) const
{
    ...
    */
    ...
    vector3 dr_torque;
    const auto& voxel_data     = mymodel->get_voxel_data();
    ...
    const matrix4f transmat =
        orientation.rotmat4() * mymodel->get_base_mesh_transformation() * matrix4f::diagonal(voxel_size);
    ...
    double finalflowforce =
        rudder.compute_force_and_torque(Fr, Tr, local_velocity, constant::SEA_WATER_DENSITY, flowforce);
    Fr.y += finalflowforce;
    ...
    */
    ...
    vector3 local_torque = tvr2.coeff_mul(area).coeff_mul(L.coeff_mul(L).coeff_mul(L))
                           * (drag_coefficient * constant::SEA_WATER_DENSITY * 0.125);
    if (w.x > 0.0) {
                ...
                while (gun_barrel != gun_turret->gun_barrels.end()) {
                    if (gun_barrel->load_time_remaining <= 0.0) {
                        ...
                        double max_shooting_distance =
                            (dist_angle_relation[gun_turret->initial_velocity].rbegin())->first;
                        ...
                        if (GUN_FIRED == res) {
                            if (!is_target_in_blindspot(gun, heading - direction)) {
                                ...
                                // shot impacted!
                                angle elevation;
                                if (calculate_gun_angle(distance, elevation, gun_turret->initial_velocity)) {
...
auto ship::is_target_in_blindspot(const struct gun_turret* gun, angle bearingToTarget) -> bool
{
...
auto ship::calculate_gun_angle(const double distance, angle& elevation, const double initial_velocity) -> bool
{
