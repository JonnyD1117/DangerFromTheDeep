...
struct sonar_contact
{
    vector2 pos;
    shipclass type;
    sonar_contact(const vector2& p, shipclass t)
        : pos(p)
        , type(t)
    {
    }
...
struct noise
{
    ...
    static double dB_to_absolute(double dB) { return (dB < 0) ? 0.0 : pow(dB_base, dB); }

    static double absolute_to_dB(double a) { return (a < 1.0) ? 0.0 : 10.0 * log10(a); }
...
struct noise_signature
{
    ...
    /** @param	distance	distance to source in meters
        ...
        @param	seastate	roughness of sea (1.0 = highest storm, 0.2=normal)
    */
    [[nodiscard]] noise compute_signal_strength(double distance, double speed, bool caviation = false) const;
};
