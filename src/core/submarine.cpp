...
submarine::submarine(date game_date, object_store<model>& model_store, const xml_elem& parent)
    : ship(game_date, model_store, parent)
    , max_depth(0)
    , dive_to(0)
    ...
    , bow_depth_rudder(
          ...
          4 /*area*/,
          10)
    , // read consts from spec file, fixme
    stern_depth_rudder(
        vector3(0, -30, 0 /*not used yet*/),
        1,
        30,
    ...
    , ballast_tank_capacity(0)
    , // read from spec file later
    ...
    , battery_level(0)
{
    ...
    // set hearing device
    if (game_date < date(1941, 6, 1)) {
        hearing_device = hearing_device_KDB;
    } else if (game_date < date(1944, 11, 1)) {
...
void submarine::load(const xml_elem& parent)
{
    ...
    electric_engine                          = (sst.attru("electric_engine") != 0U);
    snorkelup                                = (sst.attru("snorkelup") != 0U);
...
void submarine::transfer_torpedo(unsigned from, unsigned to)
{
    ...
    if (torpedoes[from].status == stored_torpedo::st_loaded && torpedoes[to].status == stored_torpedo::st_empty) {
        ...
        torpedoes[from].remaining_time = torpedoes[to].remaining_time =
            get_torp_transfer_time(from, to); // fixme: add time for torpedos already in transfer
...
void submarine::simulate(double delta_time, game& gm)
{
    ...
    for (auto& it : tanks) {
        ...
        mass_flooded_tanks += it.get_fill() * constant::SEA_WATER_DENSITY;
    }
...
{
    unsigned off =
        get_nr_of_bow_tubes() + get_nr_of_stern_tubes() + get_nr_of_bow_reserve() + get_nr_of_stern_reserve();
    return make_pair(off, off + get_nr_of_bow_deckreserve());
}

auto submarine::get_stern_deckreserve_indices() const -> pair<unsigned, unsigned>
{
...
auto submarine::get_torp_transfer_time(unsigned from, unsigned to) const -> double
{
    unsigned fl = get_location_by_tubenr(from);
    unsigned tl = get_location_by_tubenr(to);
    if (fl == 0 || tl == 0) {
        return 0.0;
    }
    if (fl == tl) {
    ...
    unsigned flin      = transl[fl];
    unsigned tlin      = transl[tl];
    if (flin > tlin) {
        unsigned tmp = flin;
        flin         = tlin;
        tlin         = tmp;
...
auto submarine::surface_visibility(const vector2& watcher) const -> float
{
    ...
    if (depth >= 0.0F && depth < 10.0F) {
        dive_factor = 0.1F * (10.0F - depth) * ship::surface_visibility(watcher);
    }
...
void submarine::ballast_tank_control_logic(double delta_time)
{
    ...
    double s1 = mass + mass_flooded_tanks - mymodel->get_base_mesh().volume * constant::SEA_WATER_DENSITY; //, s2 = 0;
...
auto submarine::get_noise_factor() const -> double
{
    ...
    noisefac = ship::get_noise_factor();
...
auto submarine::get_torp_in_tube(unsigned tubenr) const -> const submarine::stored_torpedo&
{
    if (tubenr < number_of_tubes_at[0] + number_of_tubes_at[1]) {
        return torpedoes[tubenr];
    }
...
void submarine::depth_charge_explosion(const class depth_charge& dc)
{
    ...
    double damage_radius = DAMAGE_DC_RADIUS_SURFACE + get_pos().z * DAMAGE_DC_RADIUS_200M / 200;
    double deadly_radius = DEADLY_DC_RADIUS_SURFACE + get_pos().z * DEADLY_DC_RADIUS_200M / 200;
    ...
    if (sdlen <= deadly_radius) {
        ...
        for (unsigned i = 0; i < parts.size() /*nr_of_parts*/; ++i) {
            ...
            vector3 part_center = get_pos() + vector3((tmp.x - 0.5) * bb.x, (tmp.y - 0.5) * bb.y, (tmp.z - 0.5) * bb.z);
            ...
            if (strength > 0 && strength <= 1) {
                helper::add_saturated(parts[i].status, strength, 1.0);
            }

            log_debug(
                "DC caused damage! relpos " << relpos.x << "," << relpos.y << "," << relpos.z << " dmg " << strength);
        }
    }
}
...
auto submarine::launch_torpedo(int tubenr, const vector3& targetpos, game& gm) -> bool
{
    ...
    if (tubenr < 0) { // check if target is behind
        // auto& mytarget = gm.get_object(target);
        angle a     = angle(targetpos.xy() - get_pos().xy()) - get_heading(); // + trp_addleadangle;
    ...
    if (TDC.solution_valid()) {
        ...
        doc.load();
        torpedo torp(
            gm.get_date(), gm.get_equipment_date(), gm.get_model_store(), doc.first_child(), torpedoes[tubenr].setup);
...
void submarine::compute_force_and_torque(vector3& F, vector3& T, game& gm) const
{
    ...
    vector3 Fdr;
    vector3 Tdr;
    bow_depth_rudder.compute_force_and_torque(Fdr, Tdr, get_local_velocity(), constant::SEA_WATER_DENSITY);
    ...
    double finalflowforce = stern_depth_rudder.compute_force_and_torque(
        Fdr, Tdr, get_local_velocity(), constant::SEA_WATER_DENSITY, flowforce);
    ...
    for (const auto& it : tanks) {
        double grav_force = it.get_fill() * constant::SEA_WATER_DENSITY * -constant::GRAVITY;
