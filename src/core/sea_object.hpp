...
class sea_object
{
  public:
    ...
    struct part
    {
        ...
        float floodlevel;     // how much water is inside (0-1, part of volume, 1 =
                              // full)
        // METHODS
        part(double st = -1, unsigned int rt = 0)
            : status(st)
            , repairtime(rt)
        {
        }
  ...
  protected:
    ...
    /// model to use
    /*const - cant be const yet*/ class model* mymodel{};
    ...
    /// model variants (layout / skin), read from spec file
    std::vector<skin_variant> skin_variants;

    /// skin selection data of this object [SAVE] fixme is this in model_state?!
    ...
    */
    ...
    // file.
    sea_object(std::string modelname_, object_store<model>& model_store);

    // construct a sea_object. called by heirs
    sea_object(date game_date, object_store<model>& model_store, const xml_elem& parent);

    // wether objects of that class should call visible_sea_objects to detect
    // other objects. redefine if needed.
    [[nodiscard]] virtual bool detect_other_sea_objects() const { return false; }
  ...
  public:
    ...
    [[nodiscard]] const std::string& get_specfilename() const { return specfilename; }
    [[nodiscard]] const std::string& get_modelname() const { return modelname; }
    [[nodiscard]] const std::string& get_skin_layout() const { return skin_name; }
    ...
    [[nodiscard]] virtual unsigned calc_damage() const; // returns damage in percent (100 means dead)
    ...
    [[nodiscard]] virtual const vector3& get_pos() const
    {
        return position;
    }
    ...
    [[nodiscard]] virtual double get_depth() const
    {
        return -position.z;
    }
    [[nodiscard]] virtual float get_width() const
    {
        return size3d.x;
    }
    [[nodiscard]] virtual float get_length() const
    {
        return size3d.y;
    }
    [[nodiscard]] virtual float get_height() const
    {
        return size3d.z;
    }
    [[nodiscard]] virtual float surface_visibility(const vector2& watcher) const;
    [[nodiscard]] virtual angle get_heading() const
    {
        return heading;
    }
    virtual class ai* get_ai()
    {
        return myai.get();
    }
    auto get_target()
    ...
    [[nodiscard]] const auto get_target() const
    {
        return target;
    }
    ...
    [[nodiscard]] countrycode get_country() const
    {
        return country;
    }
    [[nodiscard]] partycode get_party() const
    {
        return party;
    }
    ...
    */
    [[nodiscard]] virtual double get_noise_factor() const
    {
        return 0;
    }
    ...
    unsigned
    get_min_max_voxel_index_for_polyset(const std::vector<polygon>& polys, vector3i& vxmin, vector3i& vxmax) const;
    ...
    [[nodiscard]] double compute_collision_response_value(const vector3& collision_pos, const vector3& N) const;
