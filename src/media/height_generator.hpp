...
class height_generator
{
  public:
    ...
    */
    [[nodiscard]] const texture& get_base_texture() const { return *base_texture; }
    [[nodiscard]] const texture& get_noise_texture() const { return *noise_texture; }
    [[nodiscard]] const texture& get_sand_texture() const { return *sand_texture; }
    [[nodiscard]] const texture& get_mud_texture() const { return *mud_texture; }
    [[nodiscard]] const texture& get_grass_texture() const { return *grass_texture; }
    [[nodiscard]] const texture& get_forest_texture() const { return *forest_texture; }
    [[nodiscard]] const texture& get_rock_texture() const { return *rock_texture; }
    [[nodiscard]] const texture& get_snow_texture() const { return *snow_texture; }
    [[nodiscard]] const texture& get_forest_brdf_texture() const { return *forest_brdf_texture; }
    [[nodiscard]] const texture& get_rock_brdf_texture() const { return *rock_brdf_texture; }
    ...
    virtual void compute_normals(int detail, const vector2i& coord_bl, const vector2i& coord_sz, vector3f* dest)
    {
        ...
        for (int y = 0; y < coord_sz.y; ++y) {
            for (int x = 0; x < coord_sz.x; ++x) {
                *dest++ = vector3f(h[hli - 1] - h[hli + 1], h[hli - s2.x] - h[hli + s2.x], zh * 2).normal();
                ++hli;
            }
            hli += 2;
        }
    ...
    [[nodiscard]] unsigned get_log2_color_res_factor() const { return log2_color_res_factor; }
  ...
  protected:
    ...
    std::unique_ptr<texture> sand_texture, mud_texture, forest_texture, grass_texture, rock_texture, snow_texture,
        forest_brdf_texture, rock_brdf_texture, base_texture, noise_texture;
