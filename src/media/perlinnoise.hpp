...
class perlinnoise
{
  ...
  public:
    ...
    [[nodiscard]] unsigned get_number_of_levels() const { return noise_functions.size(); }
    ...
    [[nodiscard]] uint8_t value(unsigned x, unsigned y, unsigned depth = 0xffffffff) const;
    [[nodiscard]] float valuef(unsigned x, unsigned y, unsigned depth = 0xffffffff) const;
    [[nodiscard]] std::vector<uint8_t>
    values(unsigned x, unsigned y, unsigned w, unsigned h, unsigned depth = 0xffffffff) const;
    [[nodiscard]] std::vector<float>
    valuesf(unsigned x, unsigned y, unsigned w, unsigned h, unsigned depth = 0xffffffff) const;
};
...
class perlinnoise3d
{
  ...
  public:
    ...
    // set phase of a level
    void set_phase(unsigned func, float px, float py, float pz);

    // generate a composition of the noise functions
    std::vector<float> generate(float& minv, float& maxv) const;

    [[nodiscard]] float valuef(unsigned x, unsigned y, unsigned z, unsigned depth = 0xffffffff) const;
};
