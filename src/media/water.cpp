...
static inline unsigned nextgteqpow2(unsigned x)
{
    unsigned i = 1;
    for (; i < x && i != 0; i <<= 1) { }
    return i;
...
water::water(double tm)
    ...
    owg(wave_resolution,
        ...
        12 /*12*/ /*10*/ /*31*/,                             // wind speed m/s. fixme make dynamic
                                                             // (weather!)
        wave_resolution * (1e-8) /* roughly 2e-6 for 128 */, // scale factor for heights.
    ...
    geoclipmap_resolution(cmpdtl(cfg::instance().geti("water_detail")))
    ...
    , patches(1 + (geoclipmap_levels - 1) * 8 * 3 * 3 + 4 /*horizon*/)
{
    ...
    patches[0] = std::make_unique<geoclipmap_patch>(
        geoclipmap_resolution, 0, 0, 0, 0, geoclipmap_resolution, geoclipmap_resolution);
    unsigned N2  = geoclipmap_resolution / 2;
    unsigned N4  = geoclipmap_resolution / 4;
    unsigned N34 = geoclipmap_resolution * 3 / 4;
    for (unsigned j = 1; j < geoclipmap_levels; ++j) {
        ...
        for (int y = -1; y <= 1; ++y) {
            for (int x = -1; x <= 1; ++x) {
                ...
                patches[pn + 0] =
                    std::make_unique<geoclipmap_patch>(geoclipmap_resolution, j, 0x80, 0, 0, N4 + x, N4 + y);
                patches[pn + 1] =
                    std::make_unique<geoclipmap_patch>(geoclipmap_resolution, j, 0x01, N4 + x, 0, N2, N4 + y - 1);
                patches[pn + 2] =
                    std::make_unique<geoclipmap_patch>(geoclipmap_resolution, j, 0x40, N34 + x, 0, N4 - x, N4 + y);
                patches[pn + 3] =
                    std::make_unique<geoclipmap_patch>(geoclipmap_resolution, j, 0x02, 0, N4 + y, N4 + x - 1, N2);
                patches[pn + 4] = std::make_unique<geoclipmap_patch>(
                    geoclipmap_resolution, j, 0x08, N34 + x + 1, N4 + y, N4 - x - 1, N2);
                patches[pn + 5] =
                    std::make_unique<geoclipmap_patch>(geoclipmap_resolution, j, 0x10, 0, N34 + y, N4 + x, N4 - y);
                patches[pn + 6] = std::make_unique<geoclipmap_patch>(
                    geoclipmap_resolution, j, 0x04, N4 + x, N34 + y + 1, N2, N4 - y - 1);
                patches[pn + 7] = std::make_unique<geoclipmap_patch>(
                    geoclipmap_resolution, j, 0x20, N34 + x, N34 + y, N4 - x, N4 - y);
            }
        }
    }
    ...
    // initialize shaders
    glsl_water =
        std::make_unique<glsl_shader_setup>(get_shader_dir() + "water.vshader", get_shader_dir() + "water.fshader");
    glsl_under_water = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "under_water.vshader", get_shader_dir() + "under_water.fshader");
    glsl_water->use();
    ...
    foamamounttex =
        std::make_unique<texture>(FOAMAMOUNTRES, FOAMAMOUNTRES, GL_RGB, texture::LINEAR, texture::CLAMP, true);
    ...
    foamperimetertex = std::make_unique<texture>(
        perimetertex, perimetertexs, perimetertexs, GL_LUMINANCE_ALPHA, texture::LINEAR, texture::CLAMP);
    ...
    */
    ...
    // fast enough)
    // fixme multithreaded gives now segfault, but why on earth?!
    if (true /* construction multithreaded */) {
        auto w0 = ::thread("water-worker", [this]() {
            auto myowg(owg);
            construction_threaded(myowg, 0, 2);
        });
        auto w1 = ::thread("water-worker", [this]() {
            auto myowg(owg);
            construction_threaded(myowg, 1, 2);
        });
    } else {
...
void water::setup_textures(const matrix4& reflection_projmvmat, const vector2f& transl, bool under_water) const
{
    if (under_water) {
        glsl_under_water->use();
    } else {
    ...
    */
    static const int a_0 = 2;
    static const int b_0 = 5;
    static const int a_1 = -4;
    static const int b_1 = 3;
#ifdef WIN32
    ...
    static const double s_1 = sqrt(a_1 * a_1 + b_1 * b_1);
#endif
    static const double V_0 = 2.0;
    static const double V_1 = 1.0;
    // maybe: remove hardwired scale factors of 8 and 32, but looks best with
    // that values.
    static const double t_0 = wavetile_length / 8.0 * s_0 / V_0;
    static const double t_1 = wavetile_length / 32.0 * s_1 / V_1;
    ...
    if (!under_water) {
        ...
        glsl_water->set_uniform(
            ...
            matrix4::diagonal(1.0F / noisetilescale, 1.0F / noisetilescale, 1) * matrix4::trans(transl.x, transl.y, 0));
    }
...
void water::compute_amount_of_foam_texture(const game& gm, const vector3& viewpos, const vector<const ship*>& allships)
    const
{
    //	glPushMatrix();

    unsigned afs = foamamounttex->get_gl_width(); // size of amount of foam size, depends
                                                  // on water grid detail, fixme
    // foam is drawn the the same matrices as water, but with a smaller viewport
    if (foamamounttex_fbo != nullptr) {
...
void water::display(const vector3& viewpos, double max_view_dist, bool under_water) const
{
    ...
    vector2f transl(helper::mod(viewpos.x, double(wavetile_length)), helper::mod(viewpos.y, double(wavetile_length)));
    ...
    vector2 viewpos_mod(
        helper::mod(viewpos.x, double(wavetile_length)), helper::mod(viewpos.y, double(wavetile_length)));
    ...
    for (unsigned level = 1; level < geoclipmap_levels; ++level) {
        ...
        unsigned x_patchidx = 1;
        unsigned y_patchidx = 1;
        if (x_size[0] == N / 4 - 1) {
            x_patchidx = 0;
...
auto water::get_height(const vector2& pos) const -> float
{
    ...
    float fracx = x - ix;
    float fracy = y - iy;
    ix &= (wave_resolution - 1); // avoid rare cases of wrap-around, when x is ==
...
void water::generate_wavetile(ocean_wave_generator<float>& myowg, double tiletime, wavetile_phase& wtp)
{
    ...
    */
    ...
    for (unsigned i = 1; i < mipmap_levels; ++i) {
        wtp.mipmaps.emplace_back(wtp.mipmaps.back().wavedata, wave_resolution_shift - i, L * (1 << i));
    }
...
void water::generate_subdetail_texture()
{
    ...
    // texture
    if (water_bumpmap != nullptr) {
        // fixme: mipmap levels > 0 are not updated...
        // can we use automatic creation of mipmaps here?
        water_bumpmap->sub_image(0, 0, wave_resolution, wave_resolution, curr_wtp->mipmaps[0].normals_tex, GL_RGB);
    } else {
...
void water::set_refraction_color(const colorf& light_color)
{
    ...
    if (std::fabs(light_color.brightness() - last_light_color.brightness()) * 128 < 1.0F) {
        return;
    }
...
void water::refltex_render_unbind() const
{
    if (reflectiontex_fbo != nullptr) {
        ...
        glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 0, 0, reflectiontex->get_width(), reflectiontex->get_height(), 0);
    ...
    , use_fan(false)
{
    ...
    if ((border & 0x0f) != 0U) {
        ...
        unsigned parts = (border & 0x05) != 0U ? columns : rows; // either 0x4 or 0x1, bottom or top
    ...
    for (unsigned y = 0; y < rows; ++y) {
        if (left_to_right) {
            ...
            if (y == 0 && border == 0x20) {
                ...
                index_data[index_ptr + 1] = min_vertex_index = (level - 1) * (N + 1) * (N + 1) + N * (N + 1) + N;
            } else if (y + 1 == rows && border == 0x40) {
                // lower right patch, last line connects to corner
                index_data[index_ptr + 0] = min_vertex_index = (level - 1) * (N + 1) * (N + 1) + N;
            }
            index_ptr += 2;
            ...
            if (y == 0 && border == 0x10) {
                ...
                index_data[index_ptr - 2] = min_vertex_index = (level - 1) * (N + 1) * (N + 1);
            ...
            if (y + 1 == rows && border == 0x80) {
                ...
                index_data[index_ptr + 1] = min_vertex_index = (level - 1) * (N + 1) * (N + 1);
            ...
            if (y + 1 == rows && border == 0x40) {
                ...
                index_data[index_ptr - 1] = last_index = min_vertex_index = (level - 1) * (N + 1) * (N + 1) + N;
            }
        }
        vertex_offset += N + 1;
...
water::geoclipmap_patch::geoclipmap_patch(unsigned N, unsigned highest_level, unsigned border)
    : vbo(true)
    , min_vertex_index(0xffffffff)
    , max_vertex_index(0)
    , nr_indices(0)
