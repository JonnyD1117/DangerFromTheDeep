    ...
    , glty(0)
{
    ...
    if (texture::size_non_power_two()) {
        gltx = glty = 1;
        textures.resize(1);
        textures[0] = std::make_unique<texture>(img, 0, 0, width, height, texture::NEAREST, texture::CLAMP);
    } else {
        ...
        for (unsigned y = 0; y < glty; ++y) {
            ...
            for (unsigned x = 0; x < gltx; ++x) {
                textures[y * gltx + x] =
                    std::make_unique<texture>(img, cw, ch, widths[x], heights[y], texture::NEAREST, texture::CLAMP);
                cw += widths[x];
            }
            ch += heights[y];
        }
