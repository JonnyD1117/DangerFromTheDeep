...
bzip_streambuf::bzip_streambuf(std::ostream* os, int blocksize, int _workfactor, int _buffer_size)
    : outstream(os)
    , in_buffer(_buffer_size, 0)
    , out_buffer(_buffer_size, 0)
    , blocksize_100_k(blocksize)
    ...
    , mode(COMPRESS)
{
    ...
    bzstream.next_in   = (in_buffer).data();
    bzstream.next_out  = (out_buffer).data();
    bzstream.avail_in  = 0;
    bzstream.avail_out = 0;
    bzstream.bzalloc   = nullptr;
    bzstream.bzfree    = nullptr;

    setp(&in_buffer.front(), (in_buffer).data() + buffer_size - 1);
...
bzip_streambuf::bzip_streambuf(std::istream* is, int _buffer_size, int _put_back, int small)
    : instream(is)
    , in_buffer(_buffer_size, 0)
    , out_buffer(_buffer_size + _put_back, 0)
    , buffer_size(_buffer_size)
    , put_back(_put_back)
    , mode(DECOMPRESS)
{
    ...
    char* end = (out_buffer).data() + buffer_size + put_back;
    setg(end, end, end);
}
    ...
    if (ch != traits_type::eof()) {
        *pptr() = ch;
        ...
        do {
            ...
            outstream->write((out_buffer).data(), out_buffer.size() - bzstream.avail_out);
...
{
    instream->read((in_buffer).data(), buffer_size);
