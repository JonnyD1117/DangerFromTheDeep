...
void quadface(model::mesh* m, unsigned bv, float uscal, float vscal, bool out, int ni, int ti)
{
    ...
    m->righthanded[bv + 0] = m->righthanded[bv + 1] = m->righthanded[bv + 2] = m->righthanded[bv + 3] = 1U;
}

auto cube(float w, float l, float h, float uscal, float vscal, bool out, const string& name) -> model::mesh*
{
    ...
    for (unsigned j = 0; j < 24; ++j) {
        int i          = cnr[j];
        m->vertices[j] = vector3f((i & 1) != 0 ? w2 : -w2, (i & 2) != 0 ? l2 : -l2, (i & 4) != 0 ? h2 : -h2);
    }
    ...
    const string& name) -> model::mesh*
{
    ...
    // bugs
    m->righthanded.resize(nrvert, 1U);
    unsigned vptr   = 2;
    unsigned iptr   = 0;
    ...
    for (unsigned y = 1; y < stacks; y++) {
        ...
        for (unsigned x = 0; x <= slices; x++) {
            ...
            m->texcoords[vptr] = vector2f(uscal * float(x) / slices, vscal * float(y) / stacks);
    ...
    const string& name) -> model::mesh*
{
    ...
    unsigned basecap0 = nrvert;
    unsigned basecap1 = nrvert;
    unsigned basei0   = rsegs * 6;
    unsigned basei1   = rsegs * 6;
    if (cap && r0 > 0) {
        nrvert += (rsegs + 1) + 1;
        basecap1 = nrvert;
        basei1 += rsegs * 3;
    ...
    const string& name) -> model::mesh*
{
    ...
    for (unsigned i = 0; i <= outerrsegs; ++i) {
        ...
        for (unsigned j = 0; j <= innerrsegs; ++j) {
            ...
            float isa  = sin(iang);
            float ica  = cos(iang);
            m->vertices[i * (innerrsegs + 1) + j] =
                vector3f(ca * (outerr + ica * innerr), sa * (outerr + ica * innerr), isa * innerr);
            m->texcoords[i * (innerrsegs + 1) + j] = vector2f(uscal * j / innerrsegs, vscal * i / outerrsegs);
    ...
    for (unsigned i = 0; i < resy; ++i) {
        for (unsigned j = 0; j < resx; ++j) {
            ...
            m->vertices[i * resx + j] = vector3f(
                ...
                zscal * float(heights[i * resx + j]) * (1.0F / 255) + (r2 * 2 - 1) * znoise);
