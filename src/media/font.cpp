...
void font::print_cache()
{
    if (cache_size > 0) {
        glVertexPointer(3, GL_FLOAT, sizeof(float) * 5, cache.data());
...
{
    if (init_count == 0 && shader == nullptr) {
    ...
    unsigned maxheight) const -> unsigned
{
    ...
    unsigned textptr    = 0;
    unsigned oldtextptr = 0;
    auto textlen        = unsigned(text.length());
    int ymax            = maxheight != 0U ? int(maxheight) + y : 0x7fffffff /*intmax*/;
    while (true) {
        ...
        if (textlen > 0 && textptr == 0) { // space is not enough to wrap first
                                           // word. so disable wrapping
            w = 0xffffffff;
            continue;
        }
        if (breaktext) {
...
auto font::get_nr_of_lines_wrapped(unsigned w, const string& text, unsigned maxlines) const
    ...
    while (true) {
        ...
        while (textptr < textlen) { // remove spaces at the beginning
            ...
            if (c == '\n') {
                ...
                currwidth = 0;
                ++textptr;
                if ((maxlines != 0U) && nrlines > maxlines) {
...
{
    const auto l = unsigned(text.size());
    if (cp < l) {
        // check if we are on multibyte char
        if (is_byte_of_multibyte_char(text[cp])) {
