    ...
    : id(0)
{
    ...
    try {
        ...
        GLsizei length = 0;
        glGetShaderInfoLog(id, maxlength, &length, compilelog.data());
...
void glsl_program::link()
{
    ...
    if (waslinked == 0) {
        ...
        string compilelog(maxlength + 1, ' ');
        GLsizei length = 0;
        glGetProgramInfoLog(id, maxlength, &length, compilelog.data());
        log_warning(compilelog);
...
auto glsl_program::get_uniform_location(const std::string& name) const -> unsigned
{
    if (used_program != this) {
...
auto glsl_program::get_vertex_attrib_index(const std::string& name) const -> unsigned
{
    if (used_program != this) {
