...
class terrain : public height_generator
{
  ...
  public:
    ...
    void
    compute_heights(int, const vector2i&, const vector2i&, float*, unsigned = 0, unsigned = 0, bool = true) override;
    ...
    , noise_switch(false)
{
    ...
    noise_texture.reset(
        new texture(get_texture_dir() += "terrain_noise.dds", true, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    sand_texture.reset(
        new texture(get_texture_dir() += "terrain_sand.dds", false, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    mud_texture.reset(
        new texture(get_texture_dir() += "terrain_mud.dds", false, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    forest_texture.reset(
        new texture(get_texture_dir() += "terrain_forest.dds", false, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    grass_texture.reset(
        new texture(get_texture_dir() += "terrain_grass.dds", false, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    rock_texture.reset(
        new texture(get_texture_dir() += "terrain_rock.dds", false, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    snow_texture.reset(
        new texture(get_texture_dir() += "terrain_snow.dds", false, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    forest_brdf_texture.reset(new texture(
        get_texture_dir() += "terrain_forest_brdf.dds", false, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
    rock_brdf_texture.reset(new texture(
        get_texture_dir() += "terrain_rock_brdf.dds", false, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT));
...
bivector<float> terrain<T>::generate_patch(int detail, const vector2i& coord_bl, const vector2i& coord_sz)
{
    ...
    if (detail < (num_levels - 1)) {
        ...
        patch = generate_patch(detail + 1, coord2_bl, coord2_sz)
                    ...
                    .sub_area(offset, vector2i(coord_sz));
        ...
        patch.resize(vector2i(coord_sz));
        ...
        for (int y = 0; y < coord_sz.y; y++) {
            for (int x = 0; x < coord_sz.x; x++) {
                ...
                patch[vector2i(x, y)] =
                    m_tile_cache.get_value(vector2i(coord_geo.x + origin.x, coord_geo.y + origin.y));
            }
        }
    } else
