...
static bool do_mipmapping[texture::NR_OF_MAPPING_MODES] = {false, false, true, true, true, true};

static GLuint magfilter[texture::NR_OF_MAPPING_MODES] =
    {GL_NEAREST, GL_LINEAR, GL_NEAREST, GL_NEAREST, GL_LINEAR, GL_LINEAR};

static GLuint clampmodes[texture::NR_OF_CLAMPING_MODES] = {GL_REPEAT, GL_CLAMP_TO_EDGE};
// --------------------------------------------------
    ...
    : img(nullptr)
{
    ...
    if (extension != ".jpg|png") {
        ...
        if (teximagea->format->BytesPerPixel != 1 || teximagea->format->palette == nullptr
        ...
        SDL_Surface* result =
            SDL_CreateRGBSurface(SDL_SWSURFACE, teximagergb->w, teximagergb->h, 32, rmask, gmask, bmask, amask);
        if (result == nullptr) {
            THROW(file_read_error, filename);
        }
...
auto sdl_image::get_plain_data(unsigned& w, unsigned& h, unsigned& byte_per_pixel) -> std::vector<uint8_t>
{
    ...
    for (unsigned y = 0; y < h; ++y) {
        memcpy(&tmp[y * w * byte_per_pixel], static_cast<uint8_t*>(img->pixels) + y * img->pitch, w * byte_per_pixel);
    }
    unlock();
    return tmp;
}
    ...
    */
    ...
    if (fmt.palette != nullptr) {
        ...
        if (lumi) {
            ...
            unsigned char* offset = (static_cast<unsigned char*>(teximage->pixels)) + sy * teximage->pitch + sx;
        ...
        data.resize(tw * th * bpp);
        unsigned char* ptr    = data.data();
        unsigned char* offset = (static_cast<unsigned char*>(teximage->pixels)) + sy * teximage->pitch + sx * bpp;
        if (rgb2grey) {
            for (unsigned y = 0; y < sh; y++) {
                for (unsigned x = 0; x < sw; ++x) {
                    ptr[x * bpp] = offset[x * (bpp + 2) + 1]; // take green value, it
                }
                // doesn't matter
                if (bpp == 2) {
                    // with alpha
...
void texture::init(const vector<uint8_t>& data, bool makenormalmap, float detailh)
{
    ...
    if (makenormalmap && format == GL_LUMINANCE) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, internalformat, gl_width, gl_height, 0, format, GL_UNSIGNED_BYTE, nmpix.data());
        ...
        if (do_mipmapping[mapping]) {
            ...
            // size...
            gluBuild2DMipmaps(GL_TEXTURE_2D, format, gl_width, gl_height, format, GL_UNSIGNED_BYTE, nmpix.data());
            ...
            for (unsigned level = 1, w = gl_width / 2, h = gl_height / 2; w > 0 && h > 0; w /= 2, h /= 2) {
                ...
                glTexImage2D(GL_TEXTURE_2D, level, internalformat, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, &nmpix[0]);
                w /= 2;
                h /= 2;
            }
#endif
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, format, gl_width, gl_height, 0, format, GL_UNSIGNED_BYTE, nmpix.data());
#ifdef MEMMEASURE
        add_mem_used = gl_width * gl_height * get_bpp();
#endif
        if (do_mipmapping[mapping]) {
        ...
        switch (dimension) {
            case GL_TEXTURE_2D: {
                glTexImage2D(
                    GL_TEXTURE_2D, 0, internalformat, gl_width, gl_height, 0, format, GL_UNSIGNED_BYTE, data.data());
            } break;
            case GL_TEXTURE_1D: {
                glTexImage1D(
                    GL_TEXTURE_1D,
        ...
        if (do_mipmapping[mapping]) {
            ...
            // fixme: doesn't work with textures that don't have power of two
            // size...
            gluBuild2DMipmaps(GL_TEXTURE_2D, format, gl_width, gl_height, format, GL_UNSIGNED_BYTE, data.data());
        }
    }
    ...
    log_debug(
        "Allocated " << add_mem_used << " bytes of video memory for texture '" << texfilename
                     << "', total video mem use " << mem_used / 1024 << " kb");
...
#define MAKEFOURCC(ch0, ch1, ch2, ch3)                                                                                 \
    ((int32_t) (int8_t) (ch0) | ((int32_t) (int8_t) (ch1) << 8) | ((int32_t) (int8_t) (ch2) << 16)                     \
     | ((int32_t) (int8_t) (ch3) << 24))
void texture::load_dds(const std::string& filename, dds_data& target)
{
    ...
    file.read(reinterpret_cast<char*>((target.pixels).data()), bufferSize);
...
auto texture::scale_half(const vector<uint8_t>& src, unsigned w, unsigned h, unsigned bpp) -> vector<uint8_t>
{
    if (!size_non_power_two()) {
        if (w < 1 || (w & (w - 1)) != 0) {
    ...
    for (unsigned y = 0; y < h; y += 2) {
        for (unsigned x = 0; x < w; x += 2) {
            for (unsigned b = 0; b < bpp; ++b) {
                dst[ptr++] = uint8_t(
                    (unsigned(src[(y * w + x) * bpp + b]) + unsigned(src[(y * w + x + 1) * bpp + b])
                     + unsigned(src[((y + 1) * w + x) * bpp + b]) + unsigned(src[((y + 1) * w + x + 1) * bpp + b]))
                    / 4);
            }
        }
    }
...
auto texture::make_normals(const vector<uint8_t>& src, unsigned w, unsigned h, float detailh) -> vector<uint8_t>
{
...
auto texture::make_normals_with_alpha(const vector<uint8_t>& src, unsigned w, unsigned h, float detailh)
    -> vector<uint8_t>
{
    // src size must be w*h
    ...
    bool force_no_compression)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, internalformat, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, (void*) nullptr);
}
...
texture::texture(const std::string& filename, bool /*dummy*/, mapping_mode mapping_, clamping_mode clamp)
{
    ...
    for (int i = 0; i < image_data.numMipMaps; ++i) {
        ...
        glCompressedTexSubImage2DARB(
            GL_TEXTURE_2D, i, 0, 0, m_width, m_height, image_data.format, m_size, &image_data.pixels[m_offset]);
...
texture::~texture()
{
    ...
    log_debug(
        "Freed " << sub_mem_used << " bytes of video memory for texture '" << texfilename << "', total video mem use "
                 << mem_used / 1024 << " kb");
...
void texture::sub_image(int xoff, int yoff, unsigned w, unsigned h, const std::vector<uint8_t>& pixels, int format_)
    const
{
    glBindTexture(GL_TEXTURE_2D, opengl_name);
    glTexSubImage2D(GL_TEXTURE_2D, 0 /* mipmap level */, xoff, yoff, w, h, format_, GL_UNSIGNED_BYTE, pixels.data());
}
...
void texture::sub_image(const sdl_image& sdlimage, int xoff, int yoff, unsigned w, unsigned h)
{
    ...
    data.resize(w * h * bpp);
    unsigned char* ptr    = data.data();
    unsigned char* offset = (static_cast<unsigned char*>(teximage->pixels)) + yoff * teximage->pitch + xoff * bpp;
    for (unsigned y = 0; y < h; y++) {
        // old code, that assumes bytes come in R,G,B order:
    ...
    glBindTexture(GL_TEXTURE_2D, opengl_name);
    glTexSubImage2D(GL_TEXTURE_2D, 0 /* mipmap level */, xoff, yoff, w, h, format, GL_UNSIGNED_BYTE, data.data());
}
...
void texture::draw(int x, int y, int w, int h, const colorf& col) const
{
    ...
    primitives::textured_quad(vector2f(x, y), vector2f(x + w, y + h), *this, vector2f(0, 0), vector2f(u, v), col)
        .render();
}
...
void texture::draw_hm(int x, int y, int w, int h, const colorf& col) const
{
    float u = float(width) / gl_width;
    float v = float(height) / gl_height;
    primitives::textured_quad(vector2f(x, y), vector2f(x + w, y + h), *this, vector2f(u, 0), vector2f(0, v), col)
        .render();
}
...
void texture::draw_vm(int x, int y, int w, int h, const colorf& col) const
{
    float u = float(width) / gl_width;
    float v = float(height) / gl_height;
    primitives::textured_quad(vector2f(x, y), vector2f(x + w, y + h), *this, vector2f(0, v), vector2f(u, 0), col)
        .render();
}
...
void texture::draw_tiles(int x, int y, int w, int h, const colorf& col) const
{
    ...
    primitives::textured_quad(
        vector2f(x, y), vector2f(x + w, y + h), *this, vector2f(0, 0), vector2f(tilesx, tilesy), col)
        .render();
}
    ...
    const colorf& col) const
{
    ...
    primitives::textured_quad(vector2f(x, y), vector2f(x + w, y + h), *this, vector2f(x1, y1), vector2f(x2, y2), col)
        .render();
}
    ...
    clamping_mode clamp)
{
    ...
    glTexImage3D(
        GL_TEXTURE_3D, 0, internalformat, gl_width, gl_height, gl_depth, 0, format, GL_UNSIGNED_BYTE, pixels.data());
    ...
    if (do_mipmapping[mapping]) {
        ...
        // --matt
              ...
              // yet...
        gluBuild3DMipmaps(
            GL_TEXTURE_3D, format, gl_width, gl_height, gl_depth, format, GL_UNSIGNED_BYTE, pixels.data());
#endif
    }
...
texture3d::texture3d(unsigned w, unsigned h, unsigned d, int format_, mapping_mode mapping_, clamping_mode clamp)
{
    mapping  = mapping_;
    clamping = clamp;
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, format, w, h, d, 0, GL_RGB, GL_UNSIGNED_BYTE, (void*) nullptr);
    glBindTexture(GL_TEXTURE_3D, 0);
}
    ...
    int format_)
{
    glBindTexture(GL_TEXTURE_3D, opengl_name);
    glTexSubImage3D(
        GL_TEXTURE_3D, 0 /* mipmap level */, xoff, yoff, zoff, w, h, d, format_, GL_UNSIGNED_BYTE, pixels.data());
    glBindTexture(GL_TEXTURE_3D, 0);
}

void texture3d::draw(int x, int y, int w, int h, const vector3f& tc0, const vector3f& tcdx, const vector3f& tcdy) const
{
