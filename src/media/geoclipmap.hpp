...
class geoclipmap
{
  public:
    ...
    /// setup)
    void
    display(const frustum& f, const vector3& view_delta = vector3(), bool is_mirror = false, int above_water = 0) const;
  ...
  protected:
    ...
    struct area
    {
        ...
        area(const vector2i& a, const vector2i& b)
            : bl(a)
            , tr(b)
        {
        }
        [[nodiscard]] area intersection(const area& other) const { return area(bl.max(other.bl), tr.min(other.tr)); }
        [[nodiscard]] vector2i size() const { return {tr.x - bl.x + 1, tr.y - bl.y + 1}; }
    ...
    class level
    {
        ...
        unsigned
        generate_indices2(uint32_t* buffer, unsigned idxbase, const vector2i& size, const vector2i& vbooff) const;
        ...
        void update_VBO_and_tex(const vector2i& scratchoff, int scratchmod, const vector2i& sz, const vector2i& vbooff);
    ...
    [[nodiscard]] vector2i clamp(const vector2i& v) const { return {mod(v.x), mod(v.y)}; }
    ...
    unsigned loc_base_texture[2], loc_sand_texture[2], loc_grass_texture[2], loc_mud_texture[2], loc_forest_texture[2],
        loc_rock_texture[2], loc_snow_texture[2], loc_noise_texture[2], loc_forest_brdf_texture[2],
