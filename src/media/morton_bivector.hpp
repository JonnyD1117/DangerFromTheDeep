...
class morton_bivector
{
  ...
  public:
    ...
    [[nodiscard]] morton_bivector<T> sub_area(const vector2i& offset, const long& sz) const;
    ...
    [[nodiscard]] morton_bivector<T> downsampled(bool force_even_size = false) const;
  ...
  protected:
    ...
    [[nodiscard]] inline unsigned long coord_to_morton(const vector2i& coord) const;
};
...
morton_bivector<T> morton_bivector<T>::shifted(const vector2i& offset) const
{
    ...
    bivector_FOREACH_XYZ(
        result.at((x + offset.x) & (datasize - 1), (y + offset.y) & (datasize - 1)) = data[z]) return result;
}
...
morton_bivector<T> morton_bivector<T>::smooth_upsampled(bool wrap) const
{
    ...
    */
    ...
    if (wrap) {
        for (int y = 0; y < datasize; ++y) {
            result.at(1, 2 * y) =
                T(at(datasize - 1, y) * c1[0] + at(0, y) * c1[1] + at(1, y) * c1[2] + at(2, y) * c1[3]);
            result.at(2 * datasize - 3, 2 * y) =
                T(at(datasize - 3, y) * c1[0] + at(datasize - 2, y) * c1[1] + at(datasize - 1, y) * c1[2]
                  + at(0, y) * c1[3]);
            result.at(2 * datasize - 1, 2 * y) =
                T(at(datasize - 2, y) * c1[0] + at(datasize - 1, y) * c1[1] + at(0, y) * c1[2] + at(1, y) * c1[3]);
        }
    } else {
        for (int y = 0; y < datasize; ++y) {
    ...
    if (wrap) {
        ...
        for (int x = 0; x < resultsize; ++x) {
            result.at(x, 1) =
                T(result.at(x, 2 * datasize - 2) * c1[0] + result.at(x, 0) * c1[1] + result.at(x, 2) * c1[2]
                  + result.at(x, 4) * c1[3]);
            result.at(x, 2 * datasize - 3) =
                T(result.at(x, 2 * datasize - 6) * c1[0] + result.at(x, 2 * datasize - 4) * c1[1]
                  + result.at(x, 2 * datasize - 2) * c1[2] + result.at(x, 0) * c1[3]);
            result.at(x, 2 * datasize - 1) =
                T(result.at(x, 2 * datasize - 4) * c1[0] + result.at(x, 2 * datasize - 2) * c1[1]
        ...
        for (int x = 0; x < resultsize; ++x) {
            ...
            result.at(x, 2 * datasize - 3) =
                T(result.at(x, 2 * datasize - 6) * c1[0] + result.at(x, 2 * datasize - 4) * c1[1]
                  + result.at(x, 2 * datasize - 2) * c1[2] + result.at(x, 2 * datasize - 2) * c1[3]);
        }
    }
    return result;
}
...
morton_bivector<T>& morton_bivector<T>::add_shifted(const bivector<T>& other, const vector2i& offset)
{
    bivector_FOREACH_XY(
        at(x, y) +=
        other.at((x + offset.x) & (other.datasize - 1), (y + offset.y) & (other.datasize - 1));) return *this;
}
