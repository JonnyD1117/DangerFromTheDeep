...
const float MIN_TWINKLE_MAGNITUDE = 5.2F; // stars with mag > this value twinkle and all the other have alpha
                                          // [1.0, 0.6]
...

{
    ...
    star_positions.init_data(star_pos.size() * sizeof(vector3f), star_pos.data(), GL_STATIC_DRAW);
}
...
void stars::display(const float max_view_dist) const
{
    ...
    star_colors_VBO.init_data(star_colors.size() * sizeof(colorf), (star_colors).data(), GL_STREAM_DRAW);
