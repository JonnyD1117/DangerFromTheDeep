...
geoclipmap::geoclipmap(unsigned nr_levels, unsigned resolution_exp, height_generator& hg)
    ...
    , vboscratchbuf((resolution_vbo + 2) * (resolution_vbo + 2) * geoclipmap_fperv)
    , // 4 floats per VBO sample (x,y,z,zc)
    ...
    , wireframe(false)
{
    ...
    myshader[0] = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "geoclipmap.vshader", get_shader_dir() + "geoclipmap.fshader");
    glsl_shader::defines_list defines;
    defines.push_back("MIRROR");
    myshader[1] = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "geoclipmap.vshader", get_shader_dir() + "geoclipmap.fshader", defines);
    ...
    , outmost(outmost_level)
{
    // mostly static...
    vertices.init_data(
        gcm.resolution_vbo * gcm.resolution_vbo * geoclipmap_fperv * 4 + (outmost ? 8 * geoclipmap_fperv * 4 : 0),
        nullptr,
        GL_STATIC_DRAW);
    ...
    indices.init_data(gcm.idxscratchbuf.size() * 4 /*GLuint*/, 0, GL_STATIC_DRAW);
#endif
    ...
    normals = std::make_unique<texture>(
        pxl, gcm.resolution_vbo * 2, gcm.resolution_vbo * 2, GL_RGB, texture::LINEAR, texture::REPEAT);

    // fixme: is this still needed?
    if (color_res_fac == 2) {
        // reuse pxl
        memset(pxl.data(), index * 30, pxl.size());
    } else {
        pxl.clear();
        pxl.resize(color_res_fac * gcm.resolution_vbo * color_res_fac * gcm.resolution_vbo * 3, index * 30);
    }
}

auto geoclipmap::level::set_viewerpos(const vector3& new_viewpos, const geoclipmap::area& inner) -> geoclipmap::area
{
    ...
    area outer(
        vector2i(
            int(floor(0.5 * new_viewpos.x / L_l - 0.25 * gcm.resolution + 0.5)) * 2,
...
void geoclipmap::level::update_region(const geoclipmap::area& upar)
{
    ...
    gcm.height_gen.compute_heights(
        index, upcrd, sz + vector2i(2, 2), &gcm.vboscratchbuf[2], geoclipmap_fperv, geoclipmap_fperv * (sz.x + 2));
    unsigned ptr = 0;
    ...
    gcm.height_gen.compute_heights(
        index + 1, upcrd, szc, &gcm.vboscratchbuf[ptr + 3], 2 * geoclipmap_fperv, geoclipmap_fperv * (sz.x + 2) * 2);
    ...
    */
    ...
    gcm.height_gen.compute_normals(int(index) - 1, upar.bl * 2, sz * 2, (gcm.texnormalscratchbuf_3f).data());
    for (int y = 0; y < sz.y * 2; ++y) {
        for (int x = 0; x < sz.x * 2; ++x) {
    ...
    geoclipmap::area vboupdate(
        gcm.clamp(upar.bl - vboarea.bl + dataoffset), gcm.clamp(upar.tr - vboarea.bl + dataoffset));
    ...
    const vector2i& vbooff)
{
    ...
    */
    ...
    for (int y = 0; y < sz.y; ++y) {
        vertices.init_sub_data(
            (vbooff.x + gcm.mod(vbooff.y + y) * gcm.resolution_vbo) * geoclipmap_fperv * 4,
    ...
    const vector2i& vbooff) const -> unsigned // offset in VBO
{
    ...
    */
    ...
    vector2 minv(1e30, 1e30);
    vector2 maxv(-1e30, -1e30);
    ...
    vector2i newoffset = minvi;
    vector2i newsize   = maxvi - minvi + vector2i(1, 1);
    ...
    for (unsigned c = 0; c < cols; ++c) {
        ...
        result = generate_indices2(buffer, result, vector2i(szx, size2.y), vector2i(vbooff2.x + coloff, vbooff2.y));
        coloff += colw - 1;
    }
    return result;
#else
...
auto geoclipmap::level::generate_indices_T(uint32_t* buffer, unsigned idxbase) const -> unsigned
{
...
auto geoclipmap::level::generate_indices_horizgap(uint32_t* buffer, unsigned idxbase) const -> unsigned
{
...
void geoclipmap::level::display(const frustum& f, bool is_mirror) const
{
    ...
    vector2f texdelta_f(myfrac(texdelta.x) + 0.5 / gcm.resolution_vbo, myfrac(texdelta.y) + 0.5 / gcm.resolution_vbo);
    gcm.myshader[si]->set_uniform(gcm.loc_texcshift[si], texdelta_f);
    texdelta   = gcm.base_viewpos * (0.5 / (L_l * gcm.resolution_vbo));
    texdelta_f = vector2f(myfrac(texdelta.x) + 0.5 / gcm.resolution_vbo, myfrac(texdelta.y) + 0.5 / gcm.resolution_vbo);
    gcm.myshader[si]->set_uniform(gcm.loc_texcshift2[si], texdelta_f);

    uint32_t* indexvbo = (gcm.idxscratchbuf).data();
#ifdef DEBUG_INDEX_USAGE
...
#endif
    indices.init_sub_data(0, nridx * 4, (gcm.idxscratchbuf).data());
    ...
    glVertexAttribPointer(
        gcm.myshader_vattr_z_c_index[si], 1, GL_FLOAT, GL_FALSE, geoclipmap_fperv * 4, (float*) nullptr + 3);
