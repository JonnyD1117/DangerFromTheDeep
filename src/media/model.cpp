...
void model::render_init()
{
    ...
    glsl_plastic = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "modelrender.vshader", get_shader_dir() + "modelrender.fshader");
    ...
    glsl_color_normal_specular = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "modelrender.vshader", get_shader_dir() + "modelrender.fshader", dl);
    dl = dl2;
    ...
    glsl_mirror_clip = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "modelrender_mirrorclip.vshader", get_shader_dir() + "modelrender_mirrorclip.fshader", dl);
    ...
    , scene(0xffffffff, "<scene>", nullptr)
{
    ...
    basename = filename.substr(basepath.length(), filename.length() - basepath.length() - extension.length());
...
auto model::mesh::get_tri_iterator() const -> std::unique_ptr<model::mesh::triangle_iterator>
{
    switch (indices_type) {
        case pt_triangles:
...
void model::mesh::compute_normals()
{
    ...
    if ((mymaterial != nullptr) && (mymaterial->normalmap != nullptr)) {
        tangentsx.clear();
...
auto model::mesh::compute_tangentx(unsigned i0, unsigned i1, unsigned i2) -> bool
{
    ...
    righthanded[i0] =
        static_cast<__gnu_cxx::__alloc_traits<class std::allocator<unsigned char>, unsigned char>::value_type>(
            !(g > 0)); // fixme: negation seems needed???
    return true;
}
    ...
    , volume(0.0)
{
    ...
    for (unsigned y = 0; y < h; ++y) {
        for (unsigned x = 0; x < w; ++x) {
            vertices.push_back(
                vector3f(float(x) - rw * 0.5F, float(y) - rh * 0.5F, heights[y * w + x]).coeff_mul(scales) + trans);
...
auto model::mesh::intersects(const mesh& other, const matrix4f& transformation_this_to_other) const -> bool
{
...
auto model::mesh::is_degenerated(const vector3f& v0, const vector3f& v1, const vector3f& v2, const float eps) -> bool
{
    float eps2 = eps * eps;
...
void model::mesh::compile()
{
    ...
    if (has_texture_u0 && tangentsx.size() == vs) {
        if (mymaterial->use_default_shader()) {
            ...
            for (unsigned i = 0; i < vs; ++i) {
                ...
                xdata[4 * i + 3] = (righthanded[i]) != 0U ? 1.0F : -1.0F;
            }
            vbo_tangents_righthanded.unmap();
            vbo_tangents_righthanded.unbind();
            glsl_shader_setup& gss =
                mymaterial->specularmap != nullptr ? *glsl_color_normal_specular : *glsl_color_normal;
            gss.use();
    ...
    // later.
    index_data.init_data(indices.size() * 4 /* index type is uint32_t! */, (indices).data(), GL_STATIC_DRAW);
}
...
auto model::mesh::split(const vector3f& abc, float d) const -> pair<model::mesh*, model::mesh*>
{
...
void model::mesh::compute_adjacency()
{
    ...
    for (unsigned i = 0; i < nr_tri; ++i) {
        ...
        for (unsigned j = 0; j < 3; ++j) {
            ...
            unsigned va = std::min(v0, v1);
            unsigned vb = std::max(v0, v1);
            adjacency_edge_aux_data aa(i, j, va, vb);
            std::pair<std::set<adjacency_edge_aux_data>::iterator, bool> pib = tri_of_vertex[va].insert(aa);
...
auto model::mesh::check_adjacency() -> bool
{
...
*/
auto model::mesh::compute_inertia_tensor(const matrix4f& transmat) const -> matrix3
{
    matrix3 msum;
    ...
    bool rgb2grey)
{
    ...
    if (it != skins.end()) {
        ...
        if (it->second.ref_count == 0) {
            ...
            // itself.
            it->second.mytexture =
                new texture(basepath + it->second.filename, mapping, texture::CLAMP, makenormalmap, detailh, rgb2grey);
        }
        ++(it->second.ref_count);
    } else {
        if (ref_count == 0) {
            ...
            try {
                mytexture = std::make_unique<texture>(
                    basepath + filename, mapping, texture::CLAMP, makenormalmap, detailh, rgb2grey);
            }
            catch (std::exception& e) {
                mytexture = std::make_unique<texture>(
                    get_texture_dir() + filename, mapping, texture::CLAMP, makenormalmap, detailh, rgb2grey);
            }
        }
        ++ref_count;
    }
...
model::material::material(std::string nm)
    : name(std::move(nm))
{
}
...
void model::material::set_gl_values(const texture* caustic_map) const
{
    ...
    if (colormap != nullptr) {
        if (normalmap != nullptr) {
            ...
            // available
            if ((specularmap != nullptr) && (caustic_map == nullptr)) {
                glsl_color_normal_specular->use();
            ...
            } else if ((specularmap != nullptr) && (caustic_map != nullptr)) {
                glsl_color_normal_specular_caustic->use();
                glsl_color_normal_specular_caustic->set_gl_texture(
                    *const_cast<texture*>(caustic_map), loc_cnsc_tex_caustic, 3);
...
void model::mesh::display(const texture* caustic_map) const
{
    ...
    if (mymaterial != nullptr) {
        ...
        glsl_shader_setup::default_opaque->set_uniform(glsl_shader_setup::loc_o_color, colorf(1, 1, 1, 1));
    }
    ...
    if (has_texture_u0 && tangentsx.size() == vertices.size()) {
        if (mymaterial->use_default_shader()) {
            ...
            glVertexAttribPointer(vertex_attrib_index, 4, GL_FLOAT, GL_FALSE, 0, nullptr);
...
void model::read_phys_file(const string& filename)
{
    ...
    voxel_size = vector3f(bsize.x / voxel_resolution.x, bsize.y / voxel_resolution.y, bsize.z / voxel_resolution.z);
            ...
            for (int ixx = 0; ixx < voxel_resolution.x; ++ixx) {
                ...
                if (revvi >= 0) {
                    ...
                    for (int k = 0; k < 6; ++k) {
                        ...
                        int ny = iyy + dy[k];
                        int nz = izz + dz[k];
                        if (nx >= 0 && ny >= 0 && nz >= 0 && nx < voxel_resolution.x && ny < voxel_resolution.y
...
void model::write_color_to_dftd_model_file(xml_elem& parent, const color& c, const string& type)
{
...
auto model::read_color_from_dftd_model_file(const xml_elem& parent, const std::string& type) -> color
{
...
void model::material::map::write_to_dftd_model_file(xml_elem& parent, const std::string& type) const
{
...
model::material::map::map(const xml_elem& parent)
{
    ...
    for (auto elem : parent.iterate("skin")) {
        ...
        pair<std::map<string, skin>::iterator, bool> insok = skins.insert(make_pair(layoutname, skin()));
        if (!insok.second) {
...
static auto next_part_of_string(const string& s, string::size_type& fromwhere) -> string
{
    ...
    for (auto e : root) {
        ...
        if (etype == "material") {
            ...
            if (is_shader_material) {
                ...
                matglsl     = new material_glsl(e.attr("name"), es.attr("vertex"), es.attr("fragment"));
                mat.reset(matglsl);
            } else {
...
auto model::set_object_translation(const std::string& objname, double value) -> bool
{
    object* obj = scene.find(objname);
    if (obj == nullptr) {
        return false;
...
auto model::get_object_translation_constraints(const std::string& objname) -> vector2f
{
    object* obj = scene.find(objname);
    if (obj == nullptr) {
        return {};
...
// here it is:
auto model::get_voxels_within_sphere(const vector3f& pos, double radius) -> std::vector<unsigned>
{
    matrix4f transmat = get_base_mesh_transformation() * matrix4f::diagonal(voxel_size);
