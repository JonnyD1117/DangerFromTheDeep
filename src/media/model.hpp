...
class model
{
  public:
    ...
    class material
    {
      ...
      public:
        ...
        [[nodiscard]] virtual bool needs_texcoords() const { return colormap.get() != nullptr; }
    ...
    class material_glsl : public material
    {
      ...
      public:
        ...
        void compute_texloc();
        [[nodiscard]] const std::string& get_vertexshaderfn() const { return vertexshaderfn; }
        [[nodiscard]] const std::string& get_fragmentshaderfn() const { return fragmentshaderfn; }
        [[nodiscard]] bool needs_texcoords() const override { return nrtex > 0; }
        [[nodiscard]] bool use_default_shader() const override { return false; }
        glsl_shader_setup& get_shadersetup() { return shadersetup; }

        std::unique_ptr<map> texmaps[DFTD_MAX_TEXTURE_UNITS]; // up to DFTD_MAX_TEXTURE_UNITS
                                                              // texture units
    ...
    class mesh
    {
      public:
        ...
        // memory bandwidth.
        std::vector<uint8_t> righthanded; // a vector of bools. takes more space than a
        ...
        void get_plain_triangle(unsigned triangle, uint32_t idx[3]) const;
        void get_strip_triangle(unsigned triangle, uint32_t idx[3]) const;
  ...
  protected:
    ...
    static void write_color_to_dftd_model_file(xml_elem& parent, const color& c, const std::string& type);
    static color read_color_from_dftd_model_file(const xml_elem& parent, const std::string& type);
  ...
  public:
    ...
    */
    ...
    [[nodiscard]] unsigned get_nr_of_materials() const { return materials.size(); }
    ...
    [[nodiscard]] float get_cross_section(float angle) const; // give angle in degrees.
    ...
    */
    ...
    [[nodiscard]] const vector3i& get_voxel_resolution() const { return voxel_resolution; }
    /// request voxel size
    ...
    [[nodiscard]] float get_total_volume_by_voxels() const { return total_volume_by_voxels; }
    /// request voxel data
    [[nodiscard]] const std::vector<voxel>& get_voxel_data() const { return voxel_data; }
    ...
    [[nodiscard]] double get_bounding_sphere_radius() const { return boundsphere_radius; }
};
