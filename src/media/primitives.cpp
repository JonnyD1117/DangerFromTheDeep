...
{
    glVertexPointer(3, GL_FLOAT, sizeof(vector3f), (vertices).data());
    if (!colors.empty()) {
    ...
    if (!texcoords.empty()) {
        ...
        glTexCoordPointer(2, GL_FLOAT, sizeof(vector2f), (texcoords).data());
    }
...
auto primitives::quad(const vector2f& xy0, const vector2f& xy1, const colorf& col) -> primitive<4>
{
...
auto primitives::triangle(const vector2f& xy0, const vector2f& xy1, const vector2f& xy2, const colorf& col)
    -> primitive<3>
{
...
auto primitives::rectangle(const vector2f& xy0, const vector2f& xy1, const colorf& col) -> primitive<4>
{
...
auto primitives::diamond(const vector2f& xy, float r, const colorf& col) -> primitive<4>
{
    primitive<4> result(GL_LINE_LOOP, col);
...
auto primitives::circle(const vector2f& xy, float radius, const colorf& col) -> primitives
{
...
auto primitives::line(const vector2f& xy0, const vector2f& xy1, const colorf& col) -> primitive<2>
{
...
auto primitives::line(const vector3f& xyz0, const vector3f& xyz1, const colorf& col) -> primitive<2>
{
    primitive<2> result(GL_LINES, col);
