...
class bsplinet
{
  protected:
    ...
    T& deBoor_at(unsigned row, unsigned column) const { return deBoor_pts[(n + 1 - row) * (n - row) / 2 + column]; }
        ...
        */
        ...
        for (unsigned r = 1; r <= n; ++r) {
            for (unsigned i = l - n; i <= l - r; ++i) {
                ...
                deBoor_at(r, i + n - l) =
                    T(deBoor_at(r - 1, i + n - l) * (1 - tv) + deBoor_at(r - 1, i + 1 + n - l) * tv);
            }
        }
...
class bspline2dt
{
  ...
  public:
    ...
    bspline2dt(unsigned n_, const std::vector<T>& d)
        : n(n_)
        , cp(d)
    {
    ...
    const std::vector<T>& control_points() const
    {
        return cp;
    }
    ...
    T value(double s, double t) const
    {
        ...
        for (unsigned r = 1; r <= n; ++r) {
            for (unsigned i = l - n; i <= l - r; ++i) {
                ...
                for (unsigned j = 0; j <= n; ++j) {
                    deBoor_at(j, r, i + n - l) =
                        deBoor_at(j, r - 1, i + n - l) * (1 - tv) + deBoor_at(j, r - 1, i + 1 + n - l) * tv;
                }
            }
        }
        ...
        for (unsigned r = 1; r <= n; ++r) {
            for (unsigned i = l2 - n; i <= l2 - r; ++i) {
                ...
                deBoor_at(0, r, i + n - l2) =
                    deBoor_at(0, r - 1, i + n - l2) * (1 - tv) + deBoor_at(0, r - 1, i + 1 + n - l2) * tv;
            }
        }
