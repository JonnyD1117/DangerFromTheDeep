...
class vector3t
{
  public:
    ...
    vector3t(D x_, D y_, D z_)
        : x(x_)
        , y(y_)
        , z(z_)
    {
    }
    vector3t(const vector2t<D>& v, D z_)
        : x(v.x)
        , y(v.y)
        , z(z_)
    {
    ...
    [[nodiscard]] vector3t<D> orthogonal(const vector3t<D>& other) const { return cross(other); }
    ...
    {
        return vector3t(std::min(x, other.x), std::min(y, other.y), std::min(z, other.z));
    }

    [[nodiscard]] vector3t<D> max(const vector3t<D>& other) const
    {
        return vector3t(std::max(x, other.x), std::max(y, other.y), std::max(z, other.z));
    }
    ...
    [[nodiscard]] vector3t<D> abs() const { return vector3t(x < D(0) ? -x : x, y < D(0) ? -y : y, z < D(0) ? -z : z); }
    ...
    [[nodiscard]] vector3t<D> matrixmul(const vector3t<D>& c0, const vector3t<D>& c1, const vector3t<D>& c2) const;
    ...
    [[nodiscard]] vector4t<D> xyzw(D w) const { return vector4t<D>(x, y, z, w); }
    ...
    [[nodiscard]] vector3t<D> rcp() const { return vector3t(D(1) / x, D(1) / y, D(1) / z); }

    [[nodiscard]] D determinate(const vector3t<D>& b, const vector3t<D>& c) const
    {
        return x * b.y * c.z + b.x * c.y * z + c.x * y * b.z - x * c.y * b.z - b.x * y * c.z - c.x * b.y * z;
    }

    /// Construct axis vector
    constexpr vector3t<D>(axis a)
        : vector3t<D>()
    {
        switch (a) {
            case axis::x:
...
{
    D det = // sarrus
        o1.x * o2.y * o3.z - o1.x * o3.y * o2.z + o2.x * o3.y * o1.z - o2.x * o1.y * o3.z + o3.x * o1.y * o2.z
        - o3.x * o2.y * o1.z;
...
{
    return vector3t<D>(c0.x * x + c1.x * y + c2.x * z, c0.y * x + c1.y * y + c2.y * z, c0.z * x + c1.z * y + c2.z * z);
}
