...
auto triangulate::is_inside_triangle(const vector2& a, const vector2& b, const vector2& c, const vector2& p) -> bool
{
    double s;
    double t;
...
auto triangulate::compute(const vector<vector2>& vertices) -> vector<unsigned>
{
    ...
    while (vl_size > 3) {
        ++haengt;
        if (haengt > 8000) {
            cout << "TRIANGULATE: LOCKUP DETECTED! (" << polyscreated << "," << iscorrecttests << ","
                 << notriangpossible << ")\n";
