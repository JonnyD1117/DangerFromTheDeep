    ...
    for (; !iter.isDone(); iter.next()) {
        ...
        if (item.hasFn(MFn::kMesh)) {
            ...
            if (pathLength < item.length()) {
                ...
                ostr << "  "
                     << "<translation vector=\"" << temp[0] << " " << temp[1] << " " << temp[2] << "\"/>" << endl;
            } else {
    ...
    bool operator<(const v_unite& o) const
    {
        return (idx_vertex != o.idx_vertex)
                   ...
                   : ((idx_normal != o.idx_normal)
                          ? (idx_normal < o.idx_normal)
                          : ((idx_texcoord != o.idx_texcoord) ? (idx_texcoord < o.idx_texcoord) : false));
    }
    bool operator==(const v_unite& o) const
    {
        return idx_vertex == o.idx_vertex && idx_normal == o.idx_normal && idx_texcoord == o.idx_texcoord;
    }
};
...
void ErisianExport::readMesh(const MDagPath& dagForMesh)
{
    ...
    for (unsigned i = 1; i < v_union.size(); ++i) {
        if (v_union[k] == v_union[i]) {
            ...
            if (v_union[k].idx_vertex == v_union[i].idx_vertex && v_union[k].idx_texcoord == v_union[i].idx_texcoord
...
void ErisianExport::write()
{
    ...
    for (elem = meshList.begin(); elem != meshList.end(); elem++) {
        ostr << "  "
             << "<mesh name=\"" << elem->name << "\" id=\"" << id++ << "\" material=\"0\">" << endl;
        ostr << "    "
