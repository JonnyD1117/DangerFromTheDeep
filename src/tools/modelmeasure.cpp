...
int mymain(std::vector<string>& args)
{
    ...
    mdl = std::make_unique<model>(modelfilename, false);
    mdl->register_layout();
    mdl->set_layout();
    ...
    unsigned tm0 = SYS().millisec();
    {
        ...
        worker w0(*mdl, is_inside, resolution, 0, 2, ctr, ctrmtx, samples_per_voxel);
        worker w1(*mdl, is_inside, resolution, 1, 2, ctr, ctrmtx, samples_per_voxel);
        SYS().prepare_2d_drawing();
        while (ctr > 0) {
