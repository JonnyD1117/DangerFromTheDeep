...
class heightmap
{
  ...
  public:
    ...
    [[nodiscard]] auto heights() const -> const std::vector<float>& { return data; }
...
class camera
{
  ...
  public:
    ...
    [[nodiscard]] auto view_dir() const -> vector3 { return (look_at - position).normal(); }

    [[nodiscard]] auto look_direction() const -> angle { return angle((look_at - position).xy()); }
...
auto camera::get_transformation() const -> matrix4
{
    ...
    return {xdir.x, xdir.y, xdir.z, -p.x, ydir.x, ydir.y, ydir.z, -p.y, zdir.x, zdir.y, zdir.z, -p.z, 0, 0, 0, 1};
}
...
class canyon
{
    ...
    {
        canyon& cyn;
        explicit canyon_material(canyon& c)
            : cyn(c)
        {
        }
  ...
  public:
    ...
    /*const*/ auto get_heightdata() -> std::vector<float>& /*const*/ { return heightdata; }
};

canyon::canyon(unsigned w, unsigned h)
    : heightdata(w * h)
    , myshader(get_shader_dir() + "sandrock.vshader", get_shader_dir() + "sandrock.fshader")
{
    ...
    mymesh = std::make_unique<model::mesh>(w, h, heightdata, vector3f(2.0F, 2.0F, 0.5F), vector3f(0.0F, 0.0F, 0.0F));

    // fixme: only color here!
    sandrocktex =
        std::make_unique<texture>(get_texture_dir() + "sandrock.png", texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT);

    vector<uint8_t> noisevalues = perlinnoise(256, 2, 128).generate();
    noisetex =
        std::make_unique<texture>(noisevalues, 256, 256, GL_LUMINANCE, texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT);

    grasstex =
        std::make_unique<texture>(get_texture_dir() + "grass.png", texture::LINEAR_MIPMAP_LINEAR, texture::REPEAT);
    ...
    for (unsigned y = 0; y < h; ++y) {
        ...
        for (unsigned x = 0; x < w; ++x) {
            ...
            mymesh->texcoords[y * w + x] =
                vector2f((fx + sin(fy * 8 * 2 * M_PI) / 32) * 32 /*+fy*fx*64*/, heightdata[y * w + x] / 256);
        }
    }
#if 0
    for (unsigned y = 0; y < h; ++y) {
...
{
    cyn.myshader.use();
    cyn.myshader.set_gl_texture(*cyn.sandrocktex, cyn.loc_texsandrock, 0);
    cyn.myshader.set_gl_texture(*cyn.noisetex, cyn.loc_texnoise, 1);
    cyn.myshader.set_gl_texture(*cyn.grasstex, cyn.loc_texgrass, 2);
}
...
struct plant_alpha_sortidx
{
    ...
    auto operator<(const plant_alpha_sortidx& other) const -> bool { return sqd > other.sqd; }
};
...
plant_set::plant_set(const vector<float>& heightdata, unsigned nr, unsigned w, unsigned h, const vector2f& scal)
    : plantvertexdata(false)
    , plantindexdata(true)
    , myshader(get_shader_dir() + "billboardtrees.vshader", get_shader_dir() + "billboardtrees.fshader")
{
    float areaw = w * scal.x;
    float areah = h * scal.y;
    plants.reserve(nr);
    ...
    for (unsigned t = 0; t < nr;) {
        ...
        plants.emplace_back(vector3f(x, y, hd), vector2f(treewidth + tw, treeheight + th), rnd(plant::nr_plant_types));
    }
    planttex =
        std::make_unique<texture>(get_texture_dir() + "plants.png", texture::LINEAR_MIPMAP_LINEAR, texture::CLAMP);
    ...
    const vector2f& scal = vector2f(2.0F, 2.0F)) -> unique_ptr<model::mesh>
{
    ...
    for (unsigned t = 0; t < nr;) {
        ...
        float hd = heightdata[idxy * w + idxx] * 0.5;
        add_tree(vector3f(x, y, hd), rnd() * 90, m->vertices, m->texcoords, m->normals, m->indices);
    }
...
void show_credits()
{
    ...
    fadein_tex = std::make_unique<texture>(fadein_pixels, 8, 8, GL_LUMINANCE_ALPHA, texture::NEAREST, texture::REPEAT);
...
#define TEX3DRES 32
    ...
    while (!quit) {
        ...
        if (fadein_tex != nullptr) {
            fadein_ctr = (SYS().millisec() - tm0) * 64 / 3200;
            // generate fadein tex
            generate_fadein_pixels(fadein_pixels, fadein_ctr, 8);
            fadein_tex =
                std::make_unique<texture>(fadein_pixels, 8, 8, GL_LUMINANCE_ALPHA, texture::NEAREST, texture::REPEAT);
            glPushMatrix();
